...
void process_commands()
{
  ...
  else if(code_seen('M'))
  {
    ...
    break;
    */
 #if (defined NEW_MATHEMATICS) && (defined NEW_MATH_WITH_TESTS)  
    case 998: //M998 Test accuracy and measure math-speed
    {
      float saveresult;
      MarlinSerial ms;
      SERIAL_ECHOLN("NEW_MATHEMATICS: Performing math-tests...");
...
#if (defined MATH_SPEED_LEVEL_I) || (defined MATH_SPEED_LEVEL_II)
...
#endif //(defined MATH_SPEED_LEVEL_I) || (defined MATH_SPEED_LEVEL_II)
#if (defined MATH_SPEED_LEVEL_III) || (defined MATH_SPEED_LEVEL_IV)
      SERIAL_ECHOLN("  1.5 facos(num)   num;facos;acos");
      for (int i=-10*accuracy_factor; i < 10*accuracy_factor; i++)
      {
        float actnum = (float)i/10*accuracy_factor;
        SERIAL_ECHO("    ACOS(");
        ms.printFloat(actnum,8);
        SERIAL_ECHO(");");
        ms.printFloat(facos(actnum),8);
        SERIAL_ECHO(";");
        ms.printFloat(acos(actnum),8);
        SERIAL_ECHOLN("");
#endif //(defined MATH_SPEED_LEVEL_III)|| (defined MATH_SPEED_LEVEL_IV)
      }
      ...
      SERIAL_ECHOLN(" s");
      SERIAL_ECHOLN("");
      SERIAL_ECHOLN("  2.1a SQ(float) compared to pow(x,2) 10000 times   sq s;pow s");
      starttime1=millis();
      for (int i=0; i <= 10000; i++)
      {
        saveresult += sq((float)i/100);
      }
      stoptime1=millis();
      starttime2=millis();
      for (int i=0; i <= 10000; i++)
      {
        saveresult += pow((float)i/100,2);
      }
      stoptime2=millis();
      SERIAL_ECHO("    ");
      ms.printFloat((stoptime1-starttime1)/1000,8);
      ...
      SERIAL_ECHOLN(" s");
      SERIAL_ECHOLN("");
#if (defined MATHPEDD_LEVEL_I) || (defined MATH_SPEED_LEVEL_II)
      ...
      SERIAL_ECHOLN("  2.4 facos(num) 10000 times   facos s;acos s");
      starttime1=millis();
      for (int i=0; i <= 10000; i++)
      {
        saveresult += facos(-1+(float)i/5000);
      }
      stoptime1=millis();
      starttime2=millis();
      for (int i=0; i <= 10000; i++)
      {
        saveresult += acos(-1+(float)i/5000);
      }
      stoptime2=millis();
      SERIAL_ECHO("    ");
      ms.printFloat((stoptime1-starttime1)/1000,8);
      ...
      SERIAL_ECHOLN(" s");
      SERIAL_ECHOLN("");
      SERIAL_ECHOLN("  2.5 fatan2(opp,adj) 10000 times   facos s;acos s");
      starttime1=millis();
      for (int opp=1; opp <= 100; opp++)
      {
        for (int adj=1; adj <= 100; adj++)
        {
          saveresult += fatan2(opp,adj);
        }
      }
      stoptime1=millis();
      starttime2=millis();
    ...
    break;
#endif //(defined NEW_MATHEMATICS) && (defined NEW_MATH_WITH_TESTS)
...
#else
  SCARA_S2 = fsqrt(1-pow(SCARA_C2,2));
#endif
...
/*
#else
  SCARA_theta = (fatan2(SCARA_pos[X_AXIS],SCARA_pos[Y_AXIS])-fatan2(SCARA_K1, SCARA_K2))*-1;
  SCARA_psi = fatan2(SCARA_S2,SCARA_C2);
#endif
*/
...
#else
  float cartesian_mm = fsqrt(sq(difference[X_AXIS]) +
                             sq(difference[Y_AXIS]) +
                             sq(difference[Z_AXIS]));
#endif
